...
def handle_client_request(resource, client_socket, data):
    ...
    if resource == '/':
        ...
        if is_ok:
            ...
            num = str(num + 1)
            status_code = STATUS_CODES["ok"]
            http_header = "Content-Type: " + FILE_TYPE["txt"] + "Content-Length: " + str(len(num)) + "\r\n"
        ...
        if is_ok:
            ...
            status_code = STATUS_CODES["ok"]
            http_header = "Content-Type: " + FILE_TYPE["txt"] + "Content-Length: " + str(len(area)) + "\r\n"
            http_response = (HTTP + status_code + http_header + "\r\n")
            logging.debug("[Server]: " + http_response)
        ...
        if is_ok:
            flag = False
            file_name = "webroot/uploads/" + file_name
            photo = open(file_name, 'wb')
            photo.write(data)
            photo.close()
            status_code = STATUS_CODES["ok"]
            http_response = (HTTP + status_code + "\r\n")
            logging.debug("[Server]: " + http_response)
        ...
        if is_ok:
            ...
            if os.path.isfile(file_name):
                status_code = STATUS_CODES["ok"]
                http_header = "Content-Type: " + FILE_TYPE[file_name.split(".")[-1]]
                content_length = "Content-Length: " + str(os.path.getsize(file_name)) + "\r\n"
...
def validate_http_request(request):
    ...
    get = request.split(" ")
    http_type = ["GET", "POST"]
    if get[0] in http_type and len(get[1]) > 0 and "HTTP/1.1" in get[2] and "\r\n\r\n" in request:
        return True, request
    else:
