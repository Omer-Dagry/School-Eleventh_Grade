...
def check_process():
    ...
    try:
        conformation = encrypt("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz ,.'!-", "check_process.txt")
    ...
    if conformation == "Data was encrypted successfully!":
        ...
        if data != ENCRYPTION_CHECK:
            return tuple([False, "Encryption check failed (incorrect values)"])
...
def start():
    ...
    if sys.argv[1] == "encrypt":  # checks if the user wants to encrypt, if so call the func encrypt
        conformation = encrypt(input("Please enter the message you'd like to encrypt: "))
...
def encrypt(data, file_path="encrypted_msg.txt"):
    """
    ...
    file = open(file_path, 'w')  # open file
    for char in data:
        assert char in ENCRYPTION_VALUES, "The char '%s' can't be encrypted" % char
...
def decryption(file_path="encrypted_msg.txt"):
    ...
    for item in encrypted_data_list:
        assert item in DECRYPTION_VALUES, "The number '%s' can't be converted to anything" % item
