...
def read_from_file(file_path="encrypted_msg.txt"):
    ...
    try:
        ...
        return file.read()  # read data from "file" and returns it
    except FileNotFoundError:
        # if the file doesn't exist print a message and will exit the program
        return "There was no data to read, file does not exist."
...
def validate_data_to_encrypt(data):
    ...
    for char in data:
        if not (char in CAN_ENCRYPT):
            return "The char '%s' can't be encrypted" % char
    return "OK"
...
def validate_data_to_decrypt(data):
    ...
    for value in data_list:
        if not int(value) in CAN_DECRYPT:
            return "The value '%s' can't be decrypted" % value
    return "OK"
...
def main():
    ...
    if sys.argv[1] == "encrypt":
        ...
        if validate_msg != "OK":
            logging.warning("User input was identified as invalid, " + validate_msg)
            raise EncryptionCharError(validate_msg)
    ...
    # enter either "encrypt" or "decrypt", so if sys.argv[1] != "encrypt" it has to be equal to "decrypt"
        ...
        if encrypted_data == "There was no data to read, file does not exist.":
            raise FileNotFoundError(encrypted_data)
        logging.info("Data to decrypt: '%s'" % encrypted_data)
        validate_msg = validate_data_to_decrypt(encrypted_data)
        if validate_msg != "OK":
            logging.warning("Encrypted data in file was identifies as invalid, " + validate_msg)
            raise DecryptionValueError(validate_msg)
        decrypted_data = decryption(encrypted_data)
...
if __name__ == '__main__':
    ...
    while True:
        if len(sys.argv) >= 2:
            ...
            what_to_do = str(input("Please enter one of these options (encrypt/decrypt): "))
    ...
    while True:
        if sys.argv[1] == "encrypt" or sys.argv[1] == "decrypt":
            ...
            what_to_do = str(input("Please enter a valid option (encrypt/decrypt): "))
